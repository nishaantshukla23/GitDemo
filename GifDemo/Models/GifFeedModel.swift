...
//  GifDemo
//
//  Created by omsairam on 28/02/22.
//
...
class GifFeedModel {
    ...
    func requestFeed(_ limit: Int, offset: Int, rating: String?, terms: String?, comletionHandler:@escaping (_ succeed: Bool, _ total: Int?, _ error: String?) -> Void) {
        ...
        case .trending:
            ...
            GifNetworkManager.shared.queryTrendingGifs(limit, offset: offset, completionHandler: {(gifsResponse, error) -> Void in
                self.requesting = false
                if let gifs = gifsResponse?.gifs {
                    var newgifs = gifs
                    // check if there are duplicates
                    for newgif in newgifs {
                        if self.gifsArray.contains(where: { $0.id == newgif.id }) {
                            if let i = newgifs.firstIndex(where: { $0.id == newgif.id }) {
                                newgifs.remove(at: i)
                            }
                        }
                    }
